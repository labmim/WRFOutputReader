...
def getInformation(fileItem):
    ...
    for times in times_array:
        ...
        if (control < 24):
            status = 'Diária'
            statusn = 1
            diario.append([year, month, day, hour, julian, tempfix, u10fix, v10fix, pfix, qvfix, th2fix, raincfix, hfxfix, lhfix, swdownfix, status])
        else:
            status = 'Previsão'
            statusn = 0
            previsao.append([year, month, day, hour, julian, tempfix, u10fix, v10fix, pfix, qvfix, th2fix, raincfix, hfxfix, lhfix, swdownfix, status])
        control += 1
...
np.savetxt('series.dat', document, fmt='%s')
