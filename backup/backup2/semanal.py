...
for file in filesList:
    ...
    for times in times_array:
        ...
        temp_array = temp[:,14:15,25:26].squeeze()
        tempfix = temp_array[control:control+1][0]       

        th2_array = th2[:,14:15,25:26].squeeze()
        th2fix = th2_array[control:control+1][0] 
        
        u10_array = u10[:,14:15, 25:26].squeeze()
        u10fix = u10_array[control:control+1][0] 

        v10_array = v10[:,14:15, 25:26].squeeze()
        v10fix = v10_array[control:control+1][0]     

        p_array = p[:,14:15, 25:26].squeeze()
        pfix = p_array[control:control+1][0]

        qv_array = qv[:,14:15, 25:26].squeeze()
...
#
        ...
        if (control < 24):
            status = 'Diária'
            statusn = 1
            diario.append([year, month, day, hour, julian, tempfix, u10fix, v10fix, pfix, qvfix, th2fix])
        else:
            status = 'Previsão'
            statusn = 0
            previsao.append([year, month, day, hour, julian, tempfix, u10fix, v10fix, pfix, qvfix, th2fix])
        control += 1
        completo.append([year, month, day, hour, weekday, julian, timezone, status, tempfix, u10fix, v10fix, pfix, qvfix, cd])
...
def createDatWeekly(arquivo):
    c = []
    e = 1
    while e is not arquivo:
