...
#                urcrnrlon=-38.1598, urcrnrlat=-12.6176)
    ...
    x,y = m(lon, lat)
    #m.plot(x, y, 'bo', markersize=1, color='black')
    fixtemp = temperature[i:i+1,:,:] - 273.15
    m.contourf(x, y, np.squeeze(fixtemp), alpha = 0.4, cmap = 'GnBu') 
    cs = m.pcolor(x,y,np.squeeze(fixtemp), alpha = 0.4,cmap = 'GnBu')#, vmin=(291 - 273.15), vmax=(306  - 273.15))
    cbar = m.colorbar(cs, location='right')
...
#    m.plot(u, v, 'bo', markersize=4, color='white')
    ...
    m.plot(u, v, 'bo', markersize=4, color='white')
    plt.show()
    #plt.savefig('semanal/figdia-hour-' + str(i) + '.png', bbox_inches='tight')
    plt.close()
